        ...
        <ul>
          <router-link :to="{name:'detail', params: {id: book._id}}" v-for="(book, index) in allBooks" :key="index" tag="li">
            ...
            <div>
              ...
              <p>{{ book.bookInfo }}</p>
              <b>{{ book.bookPrice }}</b>
              <button @click.stop="remove(book._id)">删除</button>
            </div>
          </router-link>
        </ul>
      </template>
...
export default {
  ...
  methods: {
    ...
    async remove (id) {
      await removeBook(id)
      this.allBooks = this.allBooks.filter(item => item._id !== id)
    }
  },
  mounted () {
    ...
    scroll.addEventListener('touchstart', (e) => {
      ...
      let end = (e) => {
        clearInterval(this.timer1) // 函数防抖
